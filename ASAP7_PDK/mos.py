...
class MOSGenerator(DefaultCanvas):
    ...
    def __init__(self, pdk, height, fin, gate, gateDummy, shared_diff, stack, bodyswitch):
        ...
        stoppoint = self.activeOffset-self.activeWidth//2
        ...
        self.SDT = self.addGen( Wire( 'SDT', 'Sdt', 'v',
                                         clg=UncoloredCenterLineGrid( pitch=self.pdk['M1']['Pitch'], width=self.pdk['Sdt']['SdtWidth'], offset=self.pdk['M1']['Offset']),
                                         spg=EnclosureGrid( pitch=self.unitCellHeight, offset=0, stoppoint=stoppoint, check=True)))
        ...
        self.SDTb = self.addGen( Wire( 'SDTb', 'Sdt', 'v',
                                     clg=UncoloredCenterLineGrid( pitch=self.pdk['M1']['Pitch'], width= self.pdk['Sdt']['SdtWidth'], offset= self.pdk['M1']['Offset']),
                                     spg=EnclosureGrid( pitch=self.pdk['M2']['Pitch'], offset=0, stoppoint= self.pdk['M2']['Width']//2+self.pdk['V1']['VencA_L'], check=True)))

        stoppoint = self.pdk['Poly']['Pitch']//2-self.pdk['Active']['activebWidth_H']//2
        ...
        self.activeb = self.addGen( Wire( 'activeb', 'Active', 'h',
                                         clg=UncoloredCenterLineGrid( pitch=self.activePitch, width=self.pdk['Active']['activebWidth'], offset= offset_active_body),
                                         spg=EnclosureGrid( pitch=self.pdk['Poly']['Pitch'], offset=-self.pdk['Poly']['Pitch']//2, stoppoint=stoppoint, check=True)))
        ...
        for i in range((self.activeWidth-2*self.pdk['M2']['Pitch']) // v0pitch + 1):
    ...
    def _addMOS( self, x, y, x_cells,  vt_type, name='M1', reflect=False, **parameters):
        ...
        gcut_layer = self.Gcut if y ==0 else self.Gcut1
        self.addWire( gcut_layer, None, self.finsPerUnitCell*y, -self.pdk['Boundary']['TrackX'], x_cells*self.gatesPerUnitCell+2*self.shared_diff*self.gateDummy+self.pdk['Boundary']['TrackX'])
        self.addWire( self.Gcut1, None, self.finsPerUnitCell*(y+1), -self.pdk['Boundary']['TrackX'], x_cells*self.gatesPerUnitCell+2*self.shared_diff*self.gateDummy+self.pdk['Boundary']['TrackX'])
        ...
        self.addWire( self.LISD, None, gate_x + 1, (y, 1), (y+1, -1))
        self.addWire( self.SDT, None, gate_x - 1, (y, 1), (y+1, -1))
        self.addWire( self.SDT, None, gate_x + 1, (y, 1), (y+1, -1)) 
    ...
    def _addBodyContact(self, x, y, x_cells, yloc=None, name='M1'):
        ...
        self.addWire( self.SDTb, None, gate_x, ((y+1)*h+3, -1), ((y+1)*h+self.lFin//2-3, 1))
        self.addWire( self.activeb, None, y, (gate_x,1), (gate_x+1,-1))
        self.addWire( self.Gcut1, None, self.finsPerUnitCell*(y+2), -self.pdk['Boundary']['TrackX'], x_cells*self.gatesPerUnitCell+2*self.shared_diff*self.gateDummy+self.pdk['Boundary']['TrackX'])
        if self.shared_diff == 1:
        ...
        for i in range(self.gatesPerUnitCell):
            self.addWire( self.pl, None, self.gatesPerUnitCell*x+self.gateDummy*self.shared_diff+i,   (y+1,0), (y+1,1))
        ...
        if self.shared_diff == 1 and (x == 0 or x == x_cells-1):
            ...
            for i in range(self.gateDummy):
                self.addWire( self.pl, None, dummy_gates+i,   (y+1,0), (y+1,1))
